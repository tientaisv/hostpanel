...
### ⚡ 1. Cài Đặt Tự Động 1-Click Thông Minh (Khuyên Dùng)

Trình cài đặt thông minh sẽ tự động kiểm tra hệ điều hành, phân tích phần cứng (RAM) và cài đặt đầy đủ các phụ thuộc cần thiết:
- 🧠 **Tự động phát hiện cấu hình RAM**:
  - Máy chủ **< 4GB RAM**: Mặc định đề xuất & cài đặt **Podman** (siêu nhẹ, daemonless, tiết kiệm tối đa RAM). Tự động kích hoạt socket API (`podman.socket`) và liên kết `/var/run/docker.sock`.
  - Máy chủ **>= 4GB RAM**: Mặc định đề xuất & cài đặt **Docker Engine**.
- ⚙️ **Tự động kiểm tra & cài đặt phụ thuộc**: Tự động cài đặt trình biên dịch Go (Golang), Git, Curl nếu hệ thống còn thiếu.
- 🛡️ **Tích hợp Fail2ban**: Tùy chọn 1-click kích hoạt Fail2ban bảo vệ SSH & Web Server, tự động whitelist IP quản trị.
...
> **Tùy chọn dòng lệnh nâng cao:**
> - Chọn sẵn Engine: `sudo bash install.sh --engine=podman` hoặc `--engine=docker`
> - Bật sẵn Fail2ban: `sudo bash install.sh --with-fail2ban`
> - Đổi cổng Web UI mặc định: `sudo bash install.sh --port=8080`
> - Chạy tự động không hỏi (unattended): `sudo bash install.sh -y`
